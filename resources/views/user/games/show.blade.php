...
@section('title', ($game->name ?? 'Game') . ' - RanKage Game Shop')

@section('content')
<div class="container mx-auto px-4 py-4">
    <!-- Back Button -->
    <a href="/" class="inline-flex items-center text-gray-300 hover:text-primary mb-4 transition-colors">
    ...
    <div class="card mb-6 bg-gradient-to-br from-dark-card to-dark-base overflow-hidden">
        <div class="flex items-center space-x-4">
            <div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-primary to-secondary flex items-center justify-center flex-shrink-0 overflow-hidden">
                @if($game->image)
            ...
            <div class="flex-1">
                <h1 class="text-2xl font-bold text-light-text mb-1">{{ $game->name ?? 'Game' }}</h1>
                <p class="text-gray-300 text-sm">{{ $game->currency_name ?? 'Currency' }}</p>
    ...
    @php
        ...
        $jsConfig = [
            'walletBalance' => auth()->check() ? (auth()->user()->balance ?? 0) : 0,
            'packages' => $allPackages,
            'gameCode' => $gameCode ?? '',
            'requiredFields' => $requiredFields ?? [],
            'availableServers' => $availableServers ?? [],
            'fieldNotes' => $fieldNotes ?? '',
            'supportsPlayerCheck' => $supportsPlayerCheck,
            'isMLBB' => $isMLBB,
            'gameId' => $game->id ?? 0
        ];
    @endphp
    ...
    <script>
    window.gamePageConfig = JSON.parse(atob('{{ base64_encode(json_encode($jsConfig)) }}'));
</script>

    <div x-data="gameDetailData(window.gamePageConfig)">
        ...
        <div class="grid grid-cols-1 gap-4 mb-6">
            @forelse($allPackages as $package)
                @php
                    // Local packages are Eloquent models, G2Bulk packages are cast to (object)
                    $packageId = $package->id ?? null;
                    ...
                    $currencyAmount = $package->currency_amount ?? '';
                    $bonus = $package->bonus ?? 0;
                    $isG2Bulk = !empty($package->is_g2bulk);
                @endphp
                <div @click="selectPackage('{{ $packageId }}')"
                     :class="selectedPackage === '{{ $packageId }}' ? 'ring-2 ring-primary bg-primary/5' : ''"
                     class="card cursor-pointer hover:bg-dark-base transition-all">
                    <div class="flex items-center justify-between">
                        <div class="flex-1">
                            <div class="flex items-center space-x-3 mb-2">
                                <h3 class="text-lg font-bold text-light-text">{{ $packageName }}</h3>
                                @if($bonus > 0)
                                    <span class="badge badge-success">+{{ $bonus }} Bonus</span>
                                @endif
                                @if($isG2Bulk)
                            ...
                            <div class="flex items-center space-x-4">
                                <div>
                                    <p class="text-2xl font-bold text-primary">{{ $currencyAmount }}</p>
                                    <p class="text-xs text-gray-400">{{ $game->currency_name ?? 'Currency' }}</p>
                                </div>
                                <div class="text-gray-500">→</div>
                                <div>
                                    <p class="text-xl font-bold text-secondary">{{ number_format($packagePrice, 0) }}</p>
                                    <p class="text-xs text-gray-400">MMK (Ks)</p>
                                </div>
                            </div>
                        </div>
                        <div class="ml-4">
                            <div :class="selectedPackage === '{{ $packageId }}' ? 'bg-primary border-primary' : 'bg-dark-base border-dark-border'"
                                 class="w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all">
                                <svg x-show="selectedPackage === '{{ $packageId }}'" 
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" 
                                   ...
                                   required 
                                   class="input-field w-full uppercase text-white placeholder-gray-400" 
                                   placeholder="USER ID"
                                   inputmode="numeric"
                                   pattern="[0-9]*"
                        ...
                        <div>
                            ...
                            <select x-model="form.server_id" 
                                    required 
                                    class="input-field w-full text-white">
                                <option value="">Select Server</option>
                                @foreach($availableServers as $serverKey => $serverValue)
                                <option value="{{ $serverKey }}">{{ $serverValue }}</option>
                                @endforeach
                            </select>
                            @else
                            <input type="text" 
                    ...
                    <div>
                        ...
                        <input type="text" 
                               x-model="form.user_id" 
                               required 
                               class="input-field text-white placeholder-gray-400" 
                               placeholder="Enter your game User ID"
                               @if($supportsPlayerCheck)
                               @input="checkPlayerId()"
                               @blur="checkPlayerId()"
                               @endif>
                        <p class="text-xs text-gray-300 mt-1">Your game account ID</p>
                        @if($supportsPlayerCheck)
                        <!-- Player Name Display -->
                        <div x-show="playerName" x-transition class="mt-2">
                            <div class="inline-block px-3 py-1.5 bg-green-500/10 border border-green-500/30 rounded-lg">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-300 mt-1">Your game server ID</p>
                    </div>
                    @endif
                    @endif
                    ...
                    <div class="bg-dark-base rounded-xl p-4 border border-dark-border">
                        ...
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <span class="text-light-text font-semibold" x-text="selectedPackagePrice ? formatPrice(selectedPackagePrice) + ' MMK' : '-'"></span>
                        </div>
                        <div class="flex items-center justify-between pt-2 border-t border-dark-border">
                            ...
                            <span class="text-secondary text-xl font-bold" x-text="selectedPackagePrice ? formatPrice(selectedPackagePrice) + ' MMK' : '-'"></span>
                        </div>
                    </div>
...
@push('scripts')
<script>
function gameDetailData(config) {
    return {
        ...config,
        selectedPackage: null,
        selectedPackageName: '',
        selectedPackagePrice: null,
        // walletBalance: injected via config
        form: {
            user_id: '',
            server_id: '',
            payment_method: ''
        ...
        loading: false,
        error: '',
        // packages: injected via config
        // gameCode: injected via config
        // requiredFields: injected via config
        // availableServers: injected via config
        // fieldNotes: injected via config
        // supportsPlayerCheck: injected via config
        // isMLBB: injected via config
        playerName: '',
        playerCheckError: '',
        ...
        selectPackage(packageId) {
            ...
            if (pkg) {
                ...
                this.$nextTick(() => {
                    // Scroll to form after a short delay to allow transition to start
                    setTimeout(() => {
        ...
        async checkPlayerId() {
            ...
            if (this.isMLBB && (!this.form.server_id || this.form.server_id.length < 2)) {
                return;
            }
            this.checkPlayerTimeout = setTimeout(async () => {
                ...
                try {
                    ...
                    if (e.name === 'AbortError') {
                        ...
                        this.playerCheckError = 'Server error. Please refresh the page and try again.';
        ...
        async submitOrder() {
            ...
            try {
                ...
                if (e.message && e.message.includes('JSON')) {
                    this.error = 'Server error. Please refresh the page and try again.';
                    console.error('JSON parse error:', e);
                } else {
