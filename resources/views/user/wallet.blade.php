...
@section('title', 'Wallet - RanKage Game Shop')
    ...
    <div x-data="walletData()">
        ...
        <div class="card mb-6 bg-gradient-to-br from-dark-card via-dark-base to-dark-card">
            <div class="text-center">
                ...
                <p class="text-5xl font-bold text-secondary mb-4">{{ number_format(auth()->user()->balance ?? 0) }} <span class="text-2xl text-gray-400">Ks</span></p>
            ...
            <form @submit.prevent="processTopUp" enctype="multipart/form-data">
                <div class="space-y-4">
                    ...
                    <div id="kpay-payment" x-show="topUpForm.payment_method === 'kpay'" x-transition class="bg-dark-base rounded-xl p-4 border border-dark-border space-y-4">
                        ...
                        @if(isset($kpayBank) && $kpayBank)
                            <div class="bg-dark-card rounded-xl p-4 border border-dark-border">
                                <div class="flex items-center justify-between mb-3">
                                    <h4 class="font-bold text-light-text">{{ $kpayBank->bank_name }}</h4>
                                    @if($kpayBank->qr_code)
                                ...
                                <div class="space-y-2 text-sm">
                                    <div class="flex justify-between items-center py-2 border-b border-dark-border">
                                        ...
                                        <span class="text-light-text font-semibold">{{ $kpayBank->account_name }}</span>
                                    </div>
                                    <div class="flex justify-between items-center py-2 border-b border-dark-border">
                                        <span class="text-gray-400">Account Number:</span>
                                        <span class="text-light-text font-semibold font-mono">{{ $kpayBank->account_number }}</span>
                                    </div>
                                    @if($kpayBank->qr_code)
                                    <div class="flex justify-center pt-2">
                                        <img src="{{ asset('storage/' . $kpayBank->qr_code) }}" alt="QR Code" class="w-40 h-40 object-contain bg-white rounded-lg p-2">
                                    </div>
                                    @endif
                                </div>
                            </div>
                    ...
                    <div id="manual-payment" x-show="topUpForm.payment_method === 'manual'" x-transition class="bg-dark-base rounded-xl p-4 border border-dark-border space-y-4">
                        ...
                        @if(isset($activeBanks) && $activeBanks->count() > 0)
                            <div class="space-y-4">
                                ...
                                <div class="bg-dark-card rounded-xl p-4 border border-dark-border">
                                    <div class="flex items-center justify-between mb-3">
                                        <h4 class="font-bold text-light-text">{{ $bank->bank_name }}</h4>
                                        @if($bank->qr_code)
                                            <span class="badge badge-info text-xs">QR Available</span>
                                        @endif
                                    </div>
                                    <div class="space-y-2 text-sm">
                                        <div class="flex justify-between items-center py-2 border-b border-dark-border">
                                            <span class="text-gray-400">Account Name:</span>
                                            <span class="text-light-text font-semibold">{{ $bank->account_name }}</span>
                                        </div>
                                        <div class="flex justify-between items-center py-2 border-b border-dark-border">
                                            <span class="text-gray-400">Account Number:</span>
                                            <span class="text-light-text font-semibold font-mono">{{ $bank->account_number }}</span>
                                        </div>
                                        @if($bank->qr_code)
                                        <div class="flex justify-center pt-2">
                                            <img src="{{ asset('storage/' . $bank->qr_code) }}" alt="QR Code" class="w-40 h-40 object-contain bg-white rounded-lg p-2">
                                        </div>
                                        @endif
                                    </div>
                                </div>
        ...
        <div class="card">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('transactions.index') }}" class="btn-primary px-6 py-3">
