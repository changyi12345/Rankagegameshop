...
@section('title', 'API Settings - Admin Panel')
@section('page-title', 'G2Bulk API Settings')
    ...
    <div x-data="apiSettingsData()">
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between p-4 bg-dark-base rounded-xl">
                    <div>
                        ...
                        <p class="text-gray-400 text-sm">Last checked: {{ now()->format('M d, Y h:i A') }}</p>
                    </div>
        ...
        <div class="card">
            ...
            <div class="space-y-2 max-h-96 overflow-y-auto">
                @forelse($error_logs ?? [] as $log)
                <div class="bg-dark-base rounded-xl p-3 border-l-4 border-red-500">
                    <div class="flex items-start justify-between mb-1">
                        <p class="text-red-400 text-sm font-semibold">{{ $log->error_type ?? 'Error' }}</p>
                        <p class="text-gray-500 text-xs">{{ $log->created_at->format('M d, h:i A') }}</p>
                    </div>
                    <p class="text-gray-400 text-xs">{{ $log->error_message ?? $log->message ?? 'No error message available' }}</p>
                </div>
                @empty
...
function apiSettingsData() {
    return {
        showApiKey: false,
        form: {
            api_key: @json($settings->api_key ?? ''),
            api_url: @json($settings->api_url ?? 'https://api.g2bulk.com/v1'),
            callback_url: @json($settings->callback_url ?? ''),
            auto_retry: {{ $settings->auto_retry ?? false ? 'true' : 'false' }},
            max_retries: {{ $settings->max_retries ?? 3 }},
            usd_to_kyat_rate: {{ $settings->usd_to_kyat_rate ?? 2100 }}
        },
        balance: null,
        checking: false,
